...
import { Users, TrendingUp, Scroll, Book, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  if (!isOpen) {
    return (
      <div className="w-12 flex-shrink-0 border-r border-slate-700 flex flex-col items-center py-2 gap-2" data-panel="sidebar">
        <button
          onClick={() => { onToggle(true); onTabChange('party'); }}
        ...
        <button
          onClick={() => { onToggle(true); onTabChange('stats'); }}
        ...
        <button
          onClick={() => { onToggle(true); onTabChange('log'); }}
        ...
        <button
          onClick={() => { onToggle(true); onTabChange('rules'); }}
      ...
      <div className="flex border-b border-slate-700 flex-shrink-0">
        <button
          ...
          className={`flex-1 px-3 py-2 text-sm flex items-center justify-center gap-2 ${
            activeTab === 'party' ? 'bg-slate-700 text-amber-400' : 'text-slate-400 hover:bg-slate-750'
          }`}
        >
          <Users size={16} /> Party
        </button>
        <button
          ...
          className={`flex-1 px-3 py-2 text-sm flex items-center justify-center gap-2 ${
            activeTab === 'stats' ? 'bg-slate-700 text-purple-400' : 'text-slate-400 hover:bg-slate-750'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 px-3 py-2 text-sm flex items-center justify-center gap-2 ${
            activeTab === 'log' ? 'bg-slate-700 text-green-400' : 'text-slate-400 hover:bg-slate-750'
          }`}
        >
          <Scroll size={16} /> Log
        </button>
        <button
          ...
          className={`flex-1 px-3 py-2 text-sm flex items-center justify-center gap-2 ${
            activeTab === 'rules' ? 'bg-slate-700 text-blue-400' : 'text-slate-400 hover:bg-slate-750'
          }`}
        >
          <Book size={16} /> Rules
        </button>
