...
// ========== Story Beat Actions ==========
/**
 * Add a story beat (player-written or autogenerated)
 * @param {string} text - The story beat text
 * @param {string} source - 'auto' or 'player'
 */
export const addStoryBeat = (text, source = 'player') => ({
  type: A.ADD_STORY_BEAT,
  text,
  source,
  timestamp: new Date().toISOString()
});
...
/**
 * Delete a story beat by id
 * @param {number|string} id - The id of the story beat to delete
 */
export const deleteStoryBeat = (id) => ({ type: A.DEL_STORY_BEAT, id });

/**
 * Update an existing story beat's text
 * @param {number|string} id - Beat id
 * @param {string} text - New text
 */
export const updateStoryBeat = (id, text) => ({ type: A.UPD_STORY_BEAT, id, text });
